...
// @homepageURL  https://github.com/MXWXZ/Tampermonkey-Script/tree/master/VideoNoVIP/
...
(function () {
  ...
  var $ = $ || window.$;
  var interface_list = [
    { name: "接口1", url: "http://api.hlglwl.com/jx.php?url=" },
    { name: "接口2", url: "https://api.52xmw.com/?url=" },
    { name: "接口3", url: "http://vip.jlsprh.com/index.php?url=" },
  ]; // 解析接口
  ...
  var parse_div =
    "<div style='display:flex;'><div style='width:180px;padding:10px 0;'><div style='text-align:center;color:#cccccc;line-height:20px;'>解析接口</div>" +
    "<ul style='margin:0 10px;'>" +
    parse_li +
    "<div style='clear:both;'></div></ul></div></div>";
  var video_player = $(
    "<div id='iframe-div' style='width:100%;height:100%;z-index:2147483647;'><iframe id='iframe-player' frameborder='0' allowfullscreen='true' width='100%' height='100%'></iframe></div>"
  );
  ...
  if (location.href.indexOf("www.iqiyi.com") > -1) {
    ...
    // 添加按钮
    GMAddStyle(
      `.fn-iqiyi-jiexi li{color:#cccccc;text-align:center;width:60px;line-height:20px;float:left;border:1px solid gray;border-radius:8px;padding:0 4px;margin:4px 2px;}`
    );
    var iqiyi_parse = $(
      "<div class='func-item' style='z-index:2147483647;'><span class='func-inner fn-iqiyi-jiexi-text' style='line-height:40px;'><span class='func-name'>解析</span></span>" +
    ...
    var add_button_loop = setInterval(() => {
      if (
        $(".func-like-v1").is(":visible") &&
        !document.getElementsByClassName("fn-iqiyi-jiexi")[0]
      ) {
    ...
    GMAddStyle(
      `.fn-qq-jiexi li{text-align:center;width:60px;line-height:20px;float:left;border:1px solid gray;border-radius:8px;padding:0 4px;margin:4px 2px;}`
    );
    var qq_parse = $(
      "<div id='qq-jiexi-btn' class='action_item action_jiexi' style='position:relative;z-index:2147483647;'><a class='action_title fn-qq-jiexi-text'><span>解析</span></a>" +
        "<div class='mod_pop_action fn-qq-jiexi' style='background-color:#2e2e2e;width:auto;left:-50px;border:1px solid gray;'>" +
        parse_div +
        "</div></div>"
    );
    action_wrap.append(qq_parse);
    document
      .getElementById("qq-jiexi-btn")
    ...
    document
      .getElementsByClassName("fn-qq-jiexi-text")[0]
